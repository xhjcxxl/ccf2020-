# 通过语料库 训练预训练模型
import os
os.environ['TF_KERAS'] = '1'  # 必须使用 tf.keras
os.environ['CUDA_VISIBLE_DEVICES'] = '2, 3'  # 设置GPU编号
...
from pretraining_model.pretraining_corpus_building import TrainingDatasetRoBERTa
...
checkpoint_path = None  # 如果从零训练，就设置为 None
learning_rate = 0.00176
weight_decay_rate = 0.01
num_warmup_steps = 3125
num_train_steps = 125000
steps_per_epoch = 10000
grad_accum_steps = 16  # 大于1 即表明使用梯度累积（多个梯度累积到一起再计算）
epochs = num_train_steps * grad_accum_steps // steps_per_epoch
exclude_from_weight_decay = ['Norm', 'bias']
tpu_address = None  # 如果用多GPU跑，直接设置为None
...
def build_transformer_model_with_mlm():
    """
    带 mlm的bert模型
    就是 Masked Language Model(MLM) 任务
    """
...
def build_transformer_model_for_pretraining():
    """
    构建预训练模型，通用于TPU/GPU
    注意要全程使用keras标准的层写法，一些比较灵活的“移花接木”式的
    写法可能会在TPU上训练失败。此外，要注意的是TPU并非支持所有的tensorflow
    算子，尤其是不支持动态（变长）算子，因此编写相应的运算时要格外留意
    """
